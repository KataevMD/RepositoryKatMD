...
<main role="main" class="flex-shrink-0">
    ...
    <div class="container">
        <button id="createNewColl" data-toggle="modal"
                data-target="#staticBackdrop" class="btn btn-outline-secondary">Добавить новую главу
        </button>
    </div>
    <br>
    ...
    <div id="divColl" class="container text-center ">
        <data value="454253">
            <table id="CollMapTable" class="table table-hover table-responsive text-left">
                ...
                <tr>
                    <th>№</th>
                    <th class="w-75">Название главы</th>
                ...
                <c:forEach var="chapter" items="${Chapter}">
                    <tr id="tr_<c:out value="${chapter.chapter_id}"/>" class='table-data'>
                        <td><c:out value="${chapter.chapter_id}"/></td>
                        <td><c:out value="${chapter.nameChapter}"/></td>
                        <td>
                            <a href="http://localhost:8081/cstrmo/openListSectionPage?chapter_id=<c:out value='${chapter.chapter_id}'/>&nameChapter=<c:out value='${chapter.nameChapter}'/>"
                               class="btn btn-light">Просмотреть</a>
                        </td>
                        <td>
                            <button class="btn btn-light " id="viewUpdate"
                                    onclick="viewUpdate(<c:out value="${chapter.chapter_id}"/>)">
                                Редактировать
                            </button>
                        </td>
                        <td>
                            <button type="button" id="delete"
                                    onclick="deleteCollMapTableById(<c:out value="${chapter.chapter_id}"/>)"
                                    class="btn btn-light">Удалить
                            </button>
                        </td>
                    </tr>
                    <tr id="ch_<c:out value="${chapter.chapter_id}"/>" class="table-active" hidden>
                        <td>
                        </td>
                        <td>
                            <label class="w-100">
                                <input id="upCh<c:out value="${chapter.chapter_id}"/>"
<%--                                       onkeyup="checkNameColl(<c:out value="${chapter.chapter_id}"/>)"--%>
                                       value="<c:out value="${chapter.nameChapter}"/>"
                                       title="Разрешено использовать только пробелы, запятые и русские буквы"
                                       type="text" class="form-control" required>
                            </label>
                        </td>
                        <td>
                            <button id="save<c:out value="${chapter.chapter_id}"/>"
                                    class="btn btn-primary "
                                    onclick="updateChapter(<c:out value="${chapter.chapter_id}"/>)">Сохранить
                            </button>
                        </td>
                        <td>
                            <button class="btn btn-secondary "
                                    onclick="closeUpdate(<c:out value="${chapter.chapter_id}"/>)">Отмена
                            </button>
                        </td>
                        <td></td>
                    </tr>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="createForm" autocomplete="off" method="post"
                    ...
                    <label for="inputNameChapter">Введите название главы</label>
                    <input id="inputNameChapter" autocomplete="off" class="form-control" pattern="^[А-Яа-яЁё\s]+$"
                           name="nameChapter"
                           title="Разрешено использовать только пробелы, запятые и русские буквы"
